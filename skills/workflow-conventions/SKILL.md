...
description: Shared handoff contract for the multi-agent dev workflow — GATE format and checkboxes-vs-prose every stage and agent needs. Read by every workflow command and agent. Points to reference/ files (state.json schema, folder layout, git safety, OpenSpec export, test-runner detection) for detail only the stages that need it should load.
---
...
files are the only handoff. Keep produced `.md` terse in *wording*, but **never drop content to be terse**: capture
every requirement, criterion, and decision — losing one in a handoff silently breaks every later stage.

## Checkboxes vs prose

Use checkboxes (`- [ ]`) only for: lists of **steps to perform** or **conditions that must be true** (acceptance
criteria, QA steps, planned test behaviors). Everything else — descriptions, decisions, discoveries, findings,
rationale, the GATE block — is prose.
...
- `${CLAUDE_PLUGIN_ROOT}/skills/workflow-conventions/reference/state-and-layout.md` — the unit-of-work model,
  `.workflow/` folder layout, the full `state.json` schema, and Resume/Status. Read by every command that touches
  `state.json`: `start`, `propose`, `arch`, `design`, `build`.
...
- `${CLAUDE_PLUGIN_ROOT}/skills/workflow-conventions/reference/openspec-integration.md` — the OpenSpec export
  mechanics: `specRoot`, authoring, merging. Read only by `/workflow:generate-openspec` — nothing else in the
